import os

try:
    if os.environ['MEASURE_TESTING']:
        # key for testing (from https://github.com/google/oauth2client/blob/00926f2058e23da7f6772ad6477e64d7506415e5/tests/data/pem_from_pkcs12.pem) # noqa
        os.environ.setdefault("MEASURE_GOOGLE_API_JWT_PRIVATE_KEY",
                              """-----BEGIN PRIVATE KEY-----

VL5h7N0VstYhGgycuPpcIUQa
-----END PRIVATE KEY-----""")
except KeyError:
    pass
